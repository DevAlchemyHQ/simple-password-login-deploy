...
const MainContentComponent: React.FC = () => {
  ...
  return (
    ...
    >
      <div className="col-span-10 grid grid-cols-12 gap-4 h-full overflow-hidden">
        {/* Image Grid - Keep mounted but hide with CSS to prevent image reload */}
        <div className={`h-full overflow-hidden transition-[opacity,transform] duration-300 ${isExpanded ? 'hidden col-span-0' : 'block col-span-6'
          }`}>
          <ImageGrid />
        </div>

        {/* Selected Images Panel - Expand to full width when expanded */}
        <div className={`h-full overflow-hidden transition-[opacity,transform] duration-300 ${isExpanded ? 'col-span-12' : 'col-span-6'
          }`}>
          <SelectedImagesPanel
